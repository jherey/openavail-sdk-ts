...
export function registerGetEvent(server: McpServer, client: OpenavailClient): void {
  server.tool(
    'get-event',
    "Fetch a single booking by its ID. Equivalent to Google Calendar get-event. The response includes a status field: 'committed' means the booking is active; 'needs_reschedule' means it was displaced by a higher-priority event — do not retry the same slot, find a new one via check-availability.",
