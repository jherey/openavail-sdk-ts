...
export function registerDeleteEvent(server: McpServer, client: OpenavailClient): void {
  server.tool(
    ...
    [
      ...
      'RESCHEDULE ORDER: when rescheduling (moving a booking to a new time), call check-availability and secure a hold on the new slot BEFORE calling delete-event. Deleting first leaves the user with no booking if the new confirm-hold later fails.',
    ].join('\n'),
